...
def ldat(datafolder):
    ...
    return np.loadtxt(osp.join(datafolder, 'source_l_dat.tsv'),
                      dtype=np.uint8,
                      skiprows=1)
...
def sdat(datafolder):
    ...
    return np.loadtxt(osp.join(datafolder, 'source_s_dat.tsv'),
                      dtype=np.float64,
                      skiprows=1)
...
def cfldat(datafolder):
    ...
    return np.loadtxt(osp.join(datafolder, 'data_cheese_fluo.tsv'),
                      dtype=np.float64,
                      skiprows=1)
...
def cflnewdat(datafolder):
    '''Read fluorescence spectra on cheese samples and return as numpy array'''
    return np.loadtxt(osp.join(datafolder, 'data_cheese_fluo_newRand.tsv'),
                      dtype=np.float64,
                      skiprows=1)
...
def csedat(datafolder):
    ...
    return np.loadtxt(osp.join(datafolder, 'data_cheese_sensory.tsv'),
                      dtype=np.float64,
                      skiprows=1)
...
def csecol2dat(datafolder):
    ...
    mymat = np.loadtxt(osp.join(datafolder, 'data_cheese_sensory_col2.tsv'),
                       dtype=np.float64,
                       skiprows=1)
